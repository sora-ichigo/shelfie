...
export function registerBooksQueries(
  ...
  builder.queryFields((t) => ({
    ...
    userShelf: t.field({
      ...
      nullable: false,
      description:
        "Get books in another user's shelf (public accounts or following required)",
      args: {
...
export function registerBooksMutations(
  ...
  builder.mutationFields((t) => ({
    ...
    updateStartedAt: t.field({
      type: UserBookRef,
      nullable: false,
      description: "Update the start date of a book in the user's shelf",
      authScopes: {
        loggedIn: true,
      },
      args: {
      ...
      resolve: async (_parent, args, context): Promise<UserBook> => {
        ...
        const result = await shelfService.updateStartedAt({
          userBookId: args.userBookId,
          userId: userResult.data.id,
