  ...
  describe("searchByQuery", () => {
    ...
    describe("Google Books日本語フィルタリング", () => {
      ...
      it("著者不明のGoogle本は除外する", async () => {
            ...
            items: [
              {
                ...
                volumeInfo: {
                  ...
                  authors: ["著者不明"],
                  industryIdentifiers: [
                    { type: "ISBN_13" as const, identifier: "9784111111111" },
                  ],
                },
              },
              {
                ...
                volumeInfo: {
                  ...
                  authors: ["山田太郎"],
                  industryIdentifiers: [
                    { type: "ISBN_13" as const, identifier: "9784222222222" },
                  ],
                },
              },
            ],
        ...
        if (result.success) {
          expect(result.data.items).toHaveLength(1);
          expect(result.data.items[0].title).toBe("著者ありの本");
        }
      });

      it("著者が空のGoogle本は除外する", async () => {
        vi.mocked(mockRakutenRepository.searchByQuery).mockResolvedValueOnce({
          success: true,
          data: { items: [], totalItems: 0 },
        });
