...
```bash
gcloud services enable run.googleapis.com --project=shelfie-dev
gcloud services enable artifactregistry.googleapis.com --project=shelfie-dev
```
...
```bash
# 必要なロールを付与
gcloud projects add-iam-policy-binding shelfie-dev \
...
```bash
gcloud artifacts docker images list asia-northeast1-docker.pkg.dev/shelfie-dev/shelfie-api
```
