...
- [ ] 2.1 本棚画面の状態を定義する
  ...
  - 読み込み完了状態には全書籍リスト、表示用書籍リスト、検索クエリ、ソートオプション、グループ化オプションを保持
  - グループ化された書籍マップも状態に含める
  - 空判定やフィルタ済み判定のヘルパーを追加
  - _Requirements: 10.1, 10.2, 11.1, 12.1_
  - _Contracts: BookShelfState_

- [ ] 2.2 本棚画面の状態管理Notifierを実装する
  - 初期化時にmyShelfクエリを実行して本棚データを取得
  - 取得成功時はShelfStateへ同期し、読み込み完了状態へ遷移
  - 取得失敗時はエラー状態へ遷移（Failure型を使用）
  - 検索クエリ設定メソッドを実装（タイトル・著者名の部分一致フィルタ）
  - ソートオプション設定メソッドを実装（4種類のソート）
  - グループ化オプション設定メソッドを実装（3種類のグループ化）
  - データ再取得メソッドを実装
  - _Requirements: 3.3, 3.4, 4.4, 5.4, 11.1, 11.2, 12.1, 12.2, 12.3_
...
- [ ] 6.1 (P) 状態管理のユニットテストを作成する
  - 初期化時のデータ取得と状態遷移をテスト
  - 検索フィルタリングのロジック（タイトル・著者名の部分一致）をテスト
  - 各ソートオプションによる並び替え結果をテスト
  - 各グループ化オプションによるグルーピング結果をテスト
  - エラー発生時の状態遷移をテスト
  - _Requirements: 3.3, 4.4, 5.4, 11.1, 12.1_
...
- [ ] 6.4 インテグレーションテストを作成する
  ...
  - _Requirements: 3.3, 4.4, 7.1, 12.3_
