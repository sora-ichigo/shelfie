...
3. When ユーザーが検索キーワードを入力する, the 本棚画面 shall サーバーサイドでタイトルまたは著者名に一致する書籍を検索して表示する
4. If 検索結果が0件の場合, the 本棚画面 shall 「検索結果が見つかりません」メッセージを表示する
5. The 検索 shall サーバーサイドで実行され、大量データでもパフォーマンスを維持する
...
4. When ユーザーがソートオプションを選択する, the 本棚画面 shall サーバーサイドでソートされた書籍一覧を再表示する
5. The 本棚画面 shall 以下のソートオプションを提供する:
   - 追加日（新しい順）
   - 追加日（古い順）
   - タイトル（A→Z）
   - 著者名（A→Z）
6. The ソート shall サーバーサイドで実行され、ページネーションと組み合わせて正しい順序を維持する
...
### Requirement 13: ページネーション
**Objective:** As a ユーザー, I want 大量の書籍を効率的に閲覧したい, so that パフォーマンスを維持しながら全ての書籍にアクセスできる

#### Acceptance Criteria
1. The 本棚画面 shall 無限スクロールでページネーションを実装する
2. When ユーザーがリストの末尾付近までスクロールする, the 本棚画面 shall 次のページのデータを自動的に取得する
3. While 追加データを読み込み中, the 本棚画面 shall リスト末尾にローディングインジケーターを表示する
4. The ページネーション shall サーバーサイドで処理され、1ページあたり20件を取得する
5. If 全てのデータを取得済みの場合, the 本棚画面 shall 追加の読み込みを行わない
