...
- [x] 2. ドメインモデルとカードレベル判定ロジック
- [x] 2.1 (P) カードレベルとカードデータのドメイン型を定義する
...
- [x] 2.3 カードレベル判定ロジックのユニットテストを作成する
...
- [x] 3. 読了カード Widget の描画
- [x] 3.1 (P) シンプルカード（デフォルト）を描画する Widget を実装する
...
- [x] 3.2 プロフィール付きカードと感想共有カードの描画を追加する
...
- [x] 3.3* カード Widget の表示要素をウィジェットテストで検証する
...
- [x] 4. 画像キャプチャとシェア基盤
- [x] 4.1 (P) Widget をキャプチャして PNG 変換し、OS シェアシートを呼び出すサービスを実装する
...
- [x] 5. プレビュー画面とユーザー操作
- [x] 5.1 カードプレビュー画面のレイアウトとアクションバーを実装する
...
- [x] 6.2 読了変更時にシェアを促す SnackBar を表示する
...
- [x] 7. 結合テストとフロー検証
- [x] 7.1* 読了からシェアまでの一連のフローをウィジェットテストで検証する
