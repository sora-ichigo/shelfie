...
- [ ] 1. API: Firebase Auth アダプター拡張
- [ ] 1.1 (P) パスワード変更機能を Firebase REST API 連携で実装する
...
- [ ] 1.2 (P) パスワードリセットメール送信機能を Firebase REST API 連携で実装する
  - Firebase REST API の accounts:sendOobCode エンドポイントを使用
  - requestType: PASSWORD_RESET でメール送信
  - エラー発生時は適切なエラー型にマッピング
  - _Requirements: 3.2_

- [ ] 2. API: GraphQL Mutation 実装
- [ ] 2.1 changePassword Mutation を実装する
...
- [ ] 2.2 sendPasswordResetEmail Mutation を実装する
...
- [ ] 3. Mobile: ドメイン層とバリデーション
- [ ] 3.1 (P) パスワードバリデーションルールを実装する
  - 8 文字以上のチェック
  - 英字を含むことのチェック
  - 数字を含むことのチェック
  - 確認用パスワードの一致チェック
...
- [ ] 4. Mobile: データ層
- [ ] 4.1 GraphQL スキーマから Dart コードを生成する
...
- [ ] 7. Mobile: ルーティングと認証ガード
- [ ] 7.1 パスワード設定画面へのルートを追加する
