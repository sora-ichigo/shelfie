...
- [x] 9. 招待リンク（ディープリンク）の実装
- [x] 9.1 (P) Web アプリに Well-Known ファイルと招待中間ページを実装する
...
- [x] 9.2 モバイルアプリにディープリンク処理を実装する（タスク 7.4, 8.2 の完了が前提）
...
- [x] 10. プッシュ通知タップ時のルーティング拡張
- [x] 10.1 フォローリクエスト通知タップでお知らせ画面に遷移する処理を追加する（タスク 8.2 の完了が前提）
...
- [ ] 11. インフラ設定
- [ ] 11.1 (P) カスタムドメインの DNS 設定と Vercel デプロイを設定する (手動設定タスク)
