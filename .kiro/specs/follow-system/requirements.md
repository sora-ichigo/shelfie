...
本ドキュメントは、Shelfie アプリケーションにおけるフォローシステムの要件を定義する。ユーザーが他のユーザーにフォローリクエストを送り、相手が承認した場合に送信者から受信者への一方向のフォロー関係が成立する承認制フォローモデルを採用する。Instagram や X（Twitter）と同様に、相互フォローは自動では成立せず、双方がそれぞれフォローする必要がある。

プロダクト方針に基づき、プライベートな友人ネットワーク（Phase 2）として設計する。自分がフォローしていないユーザーのプロフィール詳細は閲覧できず、フォローしている相手の本棚だけが見える体験を実現する。
...
1. When ユーザーが受信したフォローリクエストを承認する, the Follow Service shall リクエストのステータスを approved に更新し、送信者から受信者への一方向のフォロー関係を成立させる
2. When ユーザーが受信したフォローリクエストを拒否する, the Follow Service shall リクエストのステータスを rejected に更新し、フォロー関係を成立させない
3. When フォローリクエストが承認された場合, the Follow Service shall 送信者のフォロー数と受信者のフォロワー数を更新する
...
6. When フォローリクエストが承認された場合, the Shelfie App shall 受信者（承認者）が送信者をまだフォローしていなければ「フォローバックする」ボタンを表示する
...
1. When ユーザーがフォロー中のユーザーに対してフォロー解除を実行する, the Follow Service shall 自分から対象ユーザーへの一方向のフォロー関係を削除する（相手から自分へのフォロー関係には影響しない）
2. When フォロー解除が成功した場合, the Follow Service shall 自分のフォロー数と対象ユーザーのフォロワー数を更新する
...
**Objective:** ユーザーとして、自分がフォローしているユーザーのプロフィールのみ閲覧したい。それにより、プライベートな友人ネットワークが維持される。

#### Acceptance Criteria

1. While ユーザーが対象ユーザーをフォローしている場合, the Shelfie App shall 対象ユーザーのプロフィール画面にアバター、表示名、ハンドル、自己紹介、フォロー数、フォロワー数、本棚情報を表示する
2. While ユーザーが対象ユーザーをフォローしている場合, the Shelfie App shall プロフィール画面にフォロー解除ボタンを表示する
3. If 自分がフォローしていないユーザーのプロフィールにアクセスしようとした場合, the Shelfie App shall 制限付きの情報（アバター、表示名、ハンドルのみ）を表示し、詳細な本棚情報を非表示にする
4. If 自分がフォローしていないユーザーのプロフィールにアクセスした場合で、かつ相手が自分をフォローしている場合, the Shelfie App shall 「フォローバックする」ボタンを表示する
5. If 自分がフォローしていないユーザーのプロフィールにアクセスした場合で、かつ相手も自分をフォローしていない場合, the Shelfie App shall フォローリクエスト送信ボタンを表示する
6. While 自分のプロフィール画面を表示している場合, the Shelfie App shall フォロー関連の操作ボタンを表示しない
...
2. The Follow Service shall フォロー関係（フォロワーID、フォロイーID、成立日時）を方向付きで永続化する。A→BとB→Aは独立したレコードとして管理する
3. The Follow Service shall 同一方向の重複リクエストおよび重複フォロー関係をデータベースレベルで防止する
